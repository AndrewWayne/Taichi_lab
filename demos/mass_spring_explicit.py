...
spring_stiffness = ti.var(ti.f32, shape=()) # 劲度系数：当振动周期小于时间分量时，则难以模拟真实场景
paused = ti.var(ti.i32, shape=())
damping = ti.var(ti.f32, shape=()) # 粘滞系数

particle_mass = 1
bottom_y = 0.05

x = ti.Vector(2, dt=ti.f32, shape=max_num_particles) # 位置矢量
v = ti.Vector(2, dt=ti.f32, shape=max_num_particles) # 速度矢量

A = ti.Matrix(2, 2, dt=ti.f32, shape=(max_num_particles, max_num_particles)) # 粒子矩阵
...
@ti.kernel
def substep(): # 前向欧拉法，使用t+1时刻的速度计算t+1时刻的位置，准确性更高
